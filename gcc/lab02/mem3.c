...
{   
    ...
    if (argc>1)
    {
        ...
        int second_h_size = atoi(argv[2]);
        printf("Размер динамически выделенной области = %d единиц\n",second_h_size);

        int* ptr2 = malloc(second_h_size * sizeof(int));
        printf("Размер динамически выделенной области (массива) = %d или %Xh байт\n", second_h_size * sizeof(int), second_h_size * sizeof(int));
        printf("Адрес  динамически выделенной области\t%Xh\n\n",ptr2);
        
         void* brk1_5 = sbrk(0);
         printf("HEAP брейк1.5-адрес\t\t\t%Xh\n", brk1_5);
         printf("HEAP брейк1.5-адрес - брейк1-адрес\t%d или %Xh байт\n\n\n", brk1_5-brk1, brk1_5-brk1);
		...
		void* brk1_75 = sbrk(0);
		printf("\nHEAP брейк1.75-адрес\t\t\t%Xh\n", brk1_75);
		printf("HEAP брейк1.75-адрес - брейк1.5-адрес\t%d или %Xh байт\n", 
        brk1_75-brk1_5, brk1_75-brk1_5);
